...
function writeLogEntry(msg: string) {
  try {
    const d = new Date();
    ...
    const fname = "app-" + y + "-" + M + "-" + D + ".log";
    const ts = y + "-" + M + "-" + D + " " + h + ":" + m + ":" + s;
    fs.appendFileSync(path.join(LOG_DIR, fname), "[" + ts + "] [INFO] " + msg + "\n", "utf-8");
  } catch {}
}
...
export function createServer(port: number = 8198) {
  const s = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/subs-check-logs') {
    const { execSync } = require('child_process');
    try {
      const out = execSync('docker logs subs-check --tail 50 2>&1').toString();
      return serveJson(res, {text: out});
    } catch (e: any) {
      return serveJson(res, {text: 'Error: ' + e.message});
    }
  }
