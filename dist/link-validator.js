 ...
 */
class LinkValidator {
    constructor(timeout = 10000, concurrency = 10) {
    ...
    async validateSingleLink(link) {
        try {
            const response = await axios_1.default.get(link.url, {
    ...
    getErrorIcon(error) {
        if (error.includes('超时'))
            return '⏱️';
        if (error.includes('域名'))
            return '🔍';
        if (error.includes('拒绝'))
            return '🚫';
